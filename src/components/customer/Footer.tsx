import Link from 'next/link';

const contact = [
  { icon: '📍', text: '196/3, Kp 2, Phường Long Khánh, Đồng Nai' },
  { icon: '☎️', text: 'Phone: 0987.654.321' },
  { icon: '📞', text: 'Zalo: 0987.654.321' },
  { icon: '✉️', text: 'nguyenhoangducthuan1605@gmail.com' },
];

const brandLinks = [
  { label: 'Giới thiệu', href: '#' },
  { label: 'Liên hệ', href: '#' },
  { label: 'Kinh nghiệm chọn hoa tươi', href: '#' },
];

const supportLinks = [
  { label: 'Chính sách và điều khoản', href: '#' },
  { label: 'Thắc mắc và khiếu nại', href: '#' },
  { label: 'Chính sách bảo mật', href: '#' },
  { label: 'Chính sách vận chuyển', href: '#' },
  { label: 'Chính sách đổi trả và hoàn tiền', href: '#' },
  { label: 'Hướng dẫn thanh toán', href: '#' },
];

const socials = [
  { label: 'Facebook', href: 'https://facebook.com', char: 'f' },
  { label: 'Instagram', href: 'https://instagram.com', char: 'in' },
  { label: 'Google', href: 'https://google.com', char: 'G' },
  { label: 'Twitter', href: 'https://twitter.com', char: 't' },
];

export default function CustomerFooter() {
  return (
    <footer className="bg-[#0f5c5c] text-white mt-12">
      <div className="container-section py-12 md:py-16">
        <div className="grid gap-10 lg:gap-14 lg:grid-cols-4">
          <div className="space-y-5">
            <div>
              <div className="text-xl font-bold text-[#f6c142] uppercase">VỀ CHÚNG TÔI</div>
              <div className="text-3xl font-bold mt-1 leading-tight">HOA TƯƠI NYNA</div>
            </div>
            <ul className="space-y-3 text-base">
              {contact.map((c) => (
                <li key={c.text} className="flex items-start gap-2">
                  <span>{c.icon}</span>
                  <span>{c.text}</span>
                </li>
              ))}
            </ul>
          </div>

          <div className="space-y-5">
            <div className="text-xl font-bold text-[#f6c142] uppercase">THƯƠNG HIỆU</div>
            <ul className="space-y-3 text-base">
              {brandLinks.map((l) => (
                <li key={l.label}>
                  <Link href={l.href} className="hover:text-[#f6c142] flex items-center gap-2">
                    <span className="text-[#f6c142]">›</span>
                    <span>{l.label}</span>
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          <div className="space-y-5">
            <div className="text-xl font-bold text-[#f6c142] uppercase">HỖ TRỢ</div>
            <ul className="space-y-3 text-base">
              {supportLinks.map((l) => (
                <li key={l.label}>
                  <Link href={l.href} className="hover:text-[#f6c142] flex items-center gap-2">
                    <span className="text-[#f6c142]">›</span>
                    <span>{l.label}</span>
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          <div className="space-y-5">
            <div className="text-xl font-bold text-[#f6c142] uppercase">THEO DÕI</div>
            <div className="flex items-center gap-3">
              {socials.map((s) => (
                <Link
                  key={s.label}
                  href={s.href}
                  className="h-12 w-12 rounded-full bg-white flex items-center justify-center text-[#0f5c5c] font-semibold text-lg shadow-sm"
                >
                  {s.char}
                </Link>
              ))}
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
}
